...
case class Map(grid: Vector[Vector[Char]], width: Int, height: Int) {
  def getTile(x: Int, y: Int): Char = {
    if (x >= 0 && x < width && y >= 0 && y < height) grid(y)(x) else '#'
  }

  def isWalkable(x: Int, y: Int): Boolean = getTile(x, y) != '#'
...
case class GameState(map: Map, player: Player, entities: Vector[Entity], running: Boolean = true)